# This is a program that would allow a child to create a cool pet, feed and exercise them and make sure they are caring for them properly
...
#Allows for a random name to be selected in a later component
import random
...
pet_names = [["Cojo", "Barney"],["Greg", "Dog"],["Darius","Hop"],["Craig","Zoom"]]
...
if __name__ == "__main__":
    ...
    print("Would you like to choose your own name or use a default name?")
    pet_name = int(input("Type 1 for default name and 2 to choose your own: "))
    if pet_name == 1:
        if virtual_pet == "dog":
            pet_name = random.choice(pet_names[0])
        elif virtual_pet == "cat":
            pet_name = random.choice(pet_names[1])
        elif virtual_pet == "bunny":
            pet_name = random.choice(pet_names[2])
        else:
            pet_name = random.choice(pet_names[3])
    else:
